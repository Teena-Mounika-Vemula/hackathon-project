    ...
    return (
        <div className="bg-white rounded-2xl p-6 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 relative">
            ...
            <div className="mt-5 flex gap-3">
                ...
                <button className="bg-slate-100 text-slate-600 font-semibold px-4 py-2 text-xs rounded-lg hover:bg-slate-200 transition-all">
                    {event.status === 'completed' ? 'Duplicate' : 'Edit'}
                </button>
            </div>
        </div>
    );
...
const EventModal = ({ isOpen, onClose, onAddEvent }) => {
    ...
    const handleSubmit = () => {
        const newEvent = {
            ...
            date: formData.date,
            time: formData.time,
            volunteers: { current: 0, max: formData.maxVolunteers },
            target: formData.description.substring(0, 50) + '...', // Simple target from description
            status: 'upcoming'
        };
        onAddEvent(newEvent);
        onClose();
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col">
                ...
                <div className="p-6 border-t flex justify-end gap-4">
                    ...
                    <button onClick={handleSubmit} className="bg-sky-500 text-white font-semibold px-5 py-2.5 rounded-xl hover:bg-sky-600">Create Event</button>
                </div>
            </div>
        </div>
    );
...
const EventsPage = () => {
    ...
    return (
        <div className="flex-1 bg-slate-50 p-4 md:p-8 overflow-y-auto">
            <EventModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onAddEvent={addEvent} />
            ...
            <div className="mb-8 flex flex-wrap gap-4 items-center justify-between">
                <button onClick={() => setIsModalOpen(true)} className="bg-sky-500 text-white font-semibold px-5 py-2.5 rounded-xl shadow-md hover:bg-sky-600 transition-all transform hover:-translate-y-0.5">
                    Add New Event
                </button>
